class TwitterApi
  ...
  def response(tag)
    ...
    tweets = TwitterApi.pull_out_tweets(json_data)
    next_url = TwitterApi.get_next_url(json_data)
    binding.pry
    return tweets, next_url
  end

  def self.next_response(url)
    HTTParty.get("https://api.twitter.com/1.1/search/tweets.json#{url}",
                { 
                  :headers => { 'Authorization' => 'Bearer AAAAAAAAAAAAAAAAAAAAACcdaQAAAAAA9Sx34TvF%2FvYRisl77b7ECdpLlV4%3DB3sKA0bm8ROQ7nsRG4RxIHhLE4EtKnf1RwIfRxTgY9iaFSwV2n' }
                 })
  end
